...
from app.schemas.schemas import (
    UserLogin, Token, UserResponse, ChangePasswordRequest
)
...
async def login(payload: UserLogin, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    # Logins are only valid against provisioned workspaces. This closes the
    # `public` pseudo-workspace as an authentication surface: it is a schema
    # (global tables) but never a workspace with legitimate user accounts.
    result = await db.execute(select(Tenant).where(Tenant.id == tenant_id))
    tenant_row = result.scalars().first()
    if not tenant_row:
        db.add(AuditLog(tenant=tenant_id, user=payload.username,
                        action="Login rejected: not a provisioned workspace.", ip=ip, status="Failed"))
        await db.commit()
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Unknown workspace - authentication rejected"
        )
    tenant_label = tenant_row.name
...
# NOTE: the old unauthenticated POST /auth/register endpoint was removed in
# the QA security pass - it allowed anyone to create accounts with arbitrary
# roles (including Super Admin). Account creation now happens exclusively
# through the admin-gated User Management API (POST /api/v1/users).
